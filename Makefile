...
run/api: build/api services/up
	...
	@./bin/api -db-dsn=${BUGBEE_DB_DSN} -smtp-username=${SMTP_USERNAME} -smtp-password=${SMTP_PASSWORD} -encryption-key=${ENCRYPTION_KEY} -smtp-sender=${SMTP_SENDER} -client-host=${CLIENT_HOST} -host=${SERVER_HOST} -client-port=${CLIENT_PORT} -client-protocol=${CLIENT_PROTOCOL} -redis-address=${REDIS_ADDRESS} -supabase-url=${SUPABASE_URL} -supabase-api-key=${SUPABASE_API_KEY}
